...
!  parameter (N_organics=32,N_inorganic=5,N_inert=2,N_liquid=12) ! YK
  ...
  ! read aerosols that needs in the output
  integer, save :: n_output_aero
  Integer, dimension(:), allocatable, save :: output_aero_index
  character (len=40), dimension(:), allocatable, save :: output_aero ! output the given aerosol concentrations

  ! read aerosols that needs in the output
  integer, save :: n_output_gas
  Integer, dimension(:), allocatable, save :: output_gas_index
  character (len=40), dimension(:), allocatable, save :: output_gas ! output the given aerosol concentrations
...
contains
  ...
  subroutine ssh_read_namelist(namelist_file)
    ...
    if (n_output_aero .gt. 0) then
        ...
        allocate(output_aero(1))
        allocate(output_aero_index(1))
    endif

    if (n_output_gas .gt. 0) then
        allocate(output_gas(n_output_gas))
        allocate(output_gas_index(n_output_gas))
    else 
        allocate(output_gas(1))
...
! genoa for testing chemistry
    ...
    do s = 1, n_output_aero
        do js = 1, N_aerosol
            if (aerosol_species_name(js) .eq. output_aero(s)) then
                ...
                if (ssh_standalone) write(*,*) 'output aero: ', trim(output_aero(s)), s, js
                if (ssh_logger) write(logfile,*) 'output aero: ', trim(output_aero(s)), s, js
            endif
        enddo
        ! check
        if (output_aero_index(s).eq.0) then
            print*,'not found aero in species list: ', s, trim(output_aero(s))
            stop
        endif
    enddo
    ...
    do s = 1, n_output_gas
        do js = 1, N_gas
            if (species_name(js) .eq. output_gas(s)) then
                ...
                if (ssh_standalone) write(*,*) 'output gas: ', trim(output_gas(s)), s, js
                if (ssh_logger) write(logfile,*) 'output gas: ', trim(output_gas(s)), s, js
            endif
        enddo
        ! check
        if (output_gas_index(s).eq.0) then
            print*,'not found gas in species list: ', s, trim(output_gas(s))
            stop
        endif
    enddo
    ...
    if (ref_soa_conc_file.ne."---") then
        ...
        ! print info
        if (ssh_standalone) write(*,*) "ierr_ref = 1. Read ref soa concentration.", nt, count,ref_soa_conc_file
        if (ssh_logger) write(logfile,*) "ierr_ref = 1. Read ref soa concentration.", nt, count,ref_soa_conc_file
    ...
    if (pre_soa_conc_file.ne."---") then
        ...
        ! print info
        if (ssh_standalone) write(*,*) "ierr_pre = 1. Read pre soa concentration.", nt, count,pre_soa_conc_file
        if (ssh_logger) write(logfile,*) "ierr_pre = 1. Read pre soa concentration.", nt, count,pre_soa_conc_file
