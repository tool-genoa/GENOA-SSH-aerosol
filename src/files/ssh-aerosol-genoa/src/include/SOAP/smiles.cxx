    ...
    if (surrogate[i].smile!="")
      {
        ...
        for (j=0; j<56; j++)
          {
            //cout<<j<<" "<<surrogate[i].smile.substr(j*9+1,8)<<" "<<atof(surrogate[i].smile.substr(j*9+1,8).c_str())<<endl;
            surrogate[i].groups[j]=atof(surrogate[i].smile.substr(j*9+1,8).c_str());
          }
